...
**구현 시 이탈(승인)**: 실제 PR #1 본문은 위 HEREDOC 보다 최신 상태를 반영해 문구를 보강함 — 필드 표 "타입·Required·설명", 토큰 치환 "쿼리/헤더/따옴표 필드 전 형태 + 비로그인 컨텍스트", `fetch-docs.sh` "(curl, 날짜 검증 후 원자적 갱신)", 계획 링크 "(리뷰 반영 이력·Tiingo 측 문서 오류 4건 포함)", 테스트 항목에 dropped 배지 4개·fixture 검증·FAQ h4 7개·"탭 안 h3 → ####"·"리뷰 9회 전부 승인" 추가. 최종 전체 리뷰 판정 "머지 가능". 후속 제안(미반영): `docs/api/README.md` 에 "알려진 문서 오류" 섹션(renderIndex 에 추가해야 재생성 시 유지됨), `LIMIT=0` 가드, `npm run check`.
